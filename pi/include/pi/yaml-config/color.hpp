...
struct YAML::convert<SDL_Color> {
    inline static YAML::Node encode(const SDL_Color& color)
    {
        using as_string = YAML::convert<std::string>;
        return as_string::encode(write_hex_rgba(color.r, color.g, color.b, color.a));
    }

    inline static bool decode(const YAML::Node& node, SDL_Color& color)
    {
...
template<std::unsigned_integral Integer>
bool pi::parse_hex_rgb(std::string_view text, Integer& r, Integer& g, Integer& b)
{
...
template<std::unsigned_integral Integer>
bool pi::parse_hex_rgba(std::string_view text, Integer& r, Integer& g, Integer& b, Integer& a)
{
